...
# --- Configuration ---
DATASET_NAME = "common_claim_true_false" 
LAYERS = [9, 20, 31]  # 0-indexed  7 (25%) 15 (50%) 23 (75%) 31 (100%)
PROMPT_A = "truthful"
PROMPT_B = "deceptive"

# SAE Configuration
MODEL_RELEASE = "gemma-scope-9b-it-res-canonical"
...
if __name__ == "__main__":
    print(f"Dataset: {DATASET_NAME}, Compare: {PROMPT_A} vs {PROMPT_B}")
    print(f"SAE: {MODEL_RELEASE}, Device: {DEVICE} \n")


    for layer in LAYERS:
        sae_id = f"layer_{layer}/width_16k/canonical"
        sae = SAEWrapper(release=MODEL_RELEASE, sae_id=sae_id, device=DEVICE)
        print(f"\n=== Analysing layer {layer}  ({sae_id}) ===")
        ...
        VIZ_DIR = RESULTS_DIR / DATASET_NAME / f"visualizations_L{LAYERS}_{PROMPT_A}_vs_{PROMPT_B}"
        RESULTS_DIR.mkdir(exist_ok=True)
        VIZ_DIR.mkdir(parents=True, exist_ok=True)

        # 2. Load activations directly to target device using corrected function
        acts_a = load_all_activations(PROMPT_A, layer, device=DEVICE)
        acts_b = load_all_activations(PROMPT_B, layer, device=DEVICE)
        ...
        # 8. Save Text Results
        out_txt = RESULTS_DIR / f"{DATASET_NAME}_L{layer}_{PROMPT_A}_vs_{PROMPT_B}_analysis.txt"
        with open(out_txt, 'w') as f:
            # (Keep the writing part as before, maybe adjust padding if needed)
            f.write(f"Dataset: {DATASET_NAME}, Layer: {layer}\n")
